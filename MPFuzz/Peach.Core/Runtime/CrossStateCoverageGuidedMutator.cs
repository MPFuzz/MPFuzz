        ...
        protected static BitStream GetMutatedValue(DataElement obj){
			...
			if((!Peach.Core.Runtime.SHARE.if_in)) {
				...
				if(dataModelsToMutate == null) {
                    ...
                    */
					if((Peach.Core.Runtime.SHARE.ifuse) && (obj.isMutable == false) && (obj is DataModel)) {
						...
						if(_dataModelToMutate.isMutable == true) {
							...
							if(ran.Next(100) < 40) //概率 有40%的概率替换
							{
								...
								if(fullname1 == fullname2) {
									...
									// Console.WriteLine("dyn:replace between states: set Value {1} from {0}", _dataModelToMutate.fullName, obj.fullName);
