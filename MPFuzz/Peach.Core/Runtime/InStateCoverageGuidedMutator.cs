...
namespace Peach.Core.Runtime
{
    public class InStateCoverageGuidedMutator
    {
        ...
        {
            BitStream coverageGuidedMutatedValue = CovGuideGetMutatedValue(obj);
            if(coverageGuidedMutatedValue != null){
				...
				if("String" == obj.elementType || "Number" == obj.elementType){
					...
					// Console.WriteLine("dyn:Reconsitution: string: {0}",(string)obj.InternalValue);
					// Console.WriteLine("dyn:Reconsitution: string: {0}",(string)coverageGuidedMutatedValue);
				}
				return true;
            }
			return false;
        ...
        protected static BitStream CovGuideGetMutatedValue(DataElement obj){
			...
			if((!Peach.Core.Runtime.SHARE.if_in)) {
				...
				if(dataModelsToMutate == null) {
					...
					// Console.WriteLine("dyn:Queue is empty,use own stratage!");
                    ...
                    */
					if((Peach.Core.Runtime.SHARE.ifuse) && (!Peach.Core.Runtime.SHARE.if_ban_guided) && (obj.isMutable == true) && !(obj is Block)) {

						// Console.WriteLine("dyn: replace among datamodels within one state");
						...
						foreach(DataElement dataElement in  _dataModelToMutate.EnumerateAllElements() ){
							...
							if( (dataElement.elementType == obj.elementType) && (result1 || result2) && (!(dataElement is Block))){
								// Console.WriteLine(" {0} {1}",dataElement.elementType,obj.elementType);

                                //todo：需要判断是否是相同值，是相同值替换没有意义 使用equal方法判断，而不是“=”
                                if(BytesArrayEquals(dataElement.Value.Value,obj.Value.Value)){
									// Console.WriteLine("dyn:Reconsitution: value equal, jump!");
                                    continue;
                                }

								// Console.WriteLine("dyn:Reconsitution: value unequal! {0} {1}",dataElement.Value.Value,obj.Value.Value);
